...
function packageReport(package_name, package_coverage) {
  ...
  for (file in package_coverage) {
    if (package_coverage.hasOwnProperty(file)) {
      ...
      branch_rate = code_branches === 0 ? 0 : (hit_branches / code_branches);
      line_rate = code_lines === 0 ? 0 : (hit_lines / code_lines);
  ...
  branch_rate = total_branches === 0 ? 0 : (total_branch_hit / total_branches);
  line_rate = total_lines === 0 ? 0 : (total_hits / total_lines);
